...
void copyPlane2Buffer(Vertex *&vertexBuffer, GLuint *&elementBuffer, unsigned int & elementStartIndex, const Vertex &a, const Vertex &b, const Vertex &c, const Vertex &d);
BoxMesh::BoxMesh(float width, float height, float depth, QColor boxColor)
{
    m_vertices.push_back(QVector3D(-0.5f*width, -0.5f*height,  0.5f*depth));
    m_vertices.push_back(QVector3D( 0.5f*width, -0.5f*height,  0.5f*depth));
    m_vertices.push_back(QVector3D( 0.5f*width,  0.5f*height,  0.5f*depth));
    m_vertices.push_back(QVector3D(-0.5f*width,  0.5f*height,  0.5f*depth));
...
void BoxMesh::copy2Buffer(Vertex *&vertexBuffer, GLuint *&elementBuffer, unsigned int &elementStartIndex) const
{
    ...
    // right plane: b=1, f=5, g=6, c=2, vertexes, (Mind: colors are numbered up)
    copyPlane2Buffer(vertexBuffer, elementBuffer, elementStartIndex,
    ...
    planeInfo.resize(6);
    //front plane: a,b,c,d, vertices(0,1,2,3)
    planeInfo[0] = Rect(m_vertices[0], m_vertices[1], m_vertices[3]);
    //right plane: b=1, f=5, g=6, c=2, vertics
