...
bool intersectsRect(const QVector3D &a, const QVector3D &b, const QVector3D &normal, const QVector3D &offset, const QVector3D &p1, const QVector3D &d, float &dist)
{
    ...
    //Condition 1: samedirection of normal vectors?
    if(angle >= 0){
    ...
    //we have three possible ways to get the intersection point, try them all umtill we succed
    double x, y;
    //rows 1 and 2
...
{
    //intersection check for triangle
    //first the normal test
