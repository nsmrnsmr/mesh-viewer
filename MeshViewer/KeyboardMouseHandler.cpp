...
void KeyboardMouseHandler::addRecongnizedKey(Qt::Key k)
{
    if(std::find(m_keys.begin(), m_keys.end(), k) != m_keys.end()) return; //already known
    //remenber key to be down and expected
    m_keys.push_back(k);
...
bool KeyboardMouseHandler::pressButton(Qt::MouseButton btn, QPoint currentPos)
{
    switch(btn){
    case Qt::LeftButton :
        m_leftButtonDown = StateHeld; break;
    case Qt::MiddleButton :
        m_middleButtonDown = StateHeld; break;
    case Qt::RightButton :
        m_rightButtonDown = StateHeld; break;
    default:
        return false;
    }
...
bool KeyboardMouseHandler::releaseButton(Qt::MouseButton btn, QPoint currentPos)
{
    switch(btn){
    case Qt::LeftButton :
        m_leftButtonDown = StateWasPressed; break;
    case Qt::MiddleButton :
        m_middleButtonDown = StateWasPressed; break;
    case Qt::RightButton :
        m_rightButtonDown = StateWasPressed; break;
    default:
        return false;
    }
...
bool KeyboardMouseHandler::buttonDown(Qt::MouseButton btn) const
{
    switch(btn){
    case Qt::LeftButton :
        return m_leftButtonDown == StateHeld;
    case Qt::MiddleButton :
        return m_middleButtonDown == StateHeld;
    case Qt::RightButton :
        return m_rightButtonDown == StateHeld;
    default:
        return false;
    }
}
...
bool KeyboardMouseHandler::buttonReleased(Qt::MouseButton btn)
{
    switch(btn){
    case Qt::LeftButton :
        return m_leftButtonDown == StateWasPressed;
    case Qt::MiddleButton :
        return m_middleButtonDown == StateWasPressed;
    case Qt::RightButton :
        return m_rightButtonDown == StateWasPressed;
...
void KeyboardMouseHandler::clearWasPressedKeyStates()
{
    ...
    for(unsigned int i=0; i<m_keyStates.size(); ++i){
        m_keyStates[i] = static_cast<KeyStates>(m_keyStates[i] & 1); //toggle "WasPress" bit -> NotPress
    }
}
